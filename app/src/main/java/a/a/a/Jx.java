...
public class Jx {
    ...
    public String generateCode(boolean isAndroidStudioExport, String sc_id) {
        boolean isDialogFragment = (projectFileBean.fileName.contains("_dialog_fragment")
                || DayDreamProjectSettings.getActivityType(Configs.currentProjectID, projectFileBean.fileName).contains("_dialog_fragment"));

        boolean isBottomDialogFragment = (projectFileBean.fileName.contains("_bottomdialog_fragment")
                || DayDreamProjectSettings.getActivityType(Configs.currentProjectID, projectFileBean.fileName).contains("_bottomdialog_fragment"));
        ...
        if (isFragment) {
            ...
            if (DRFeatureManager.isEdgeToEdgeEnabled(sc_id, projectFileBean.fileName))
            ...
            if (DRFeatureManager.isWindowInsetsHandlingEnabled(sc_id, projectFileBean.fileName)) {
        ...
        if (!isFragment) {
            if (DayDreamProjectSettings.isEnableDayDream(sc_id)) {
                if (DRFeatureManager.isContentProtectionEnabled(sc_id, projectFileBean.fileName))
                    sb.append("getWindow().setFlags(WindowManager.LayoutParams.FLAG_SECURE, WindowManager.LayoutParams.FLAG_SECURE);").append(EOL);

                if (DRFeatureManager.isOnBackInvokedCallbackEnabled(sc_id, projectFileBean.fileName)) {
    ...
    private void handleAppCompat() {
        ...
        if (ProjectLibrary.isEnabledAppCompat(Configs.currentProjectID)) {
            if (DRFeatureManager.isEdgeToEdgeEnabled(Configs.currentProjectID, projectFileBean.fileName))
                addImport("androidx.activity.EdgeToEdge");

            if (DRFeatureManager.isWindowInsetsHandlingEnabled(Configs.currentProjectID, projectFileBean.fileName)) {
            ...
            if (DayDreamProjectSettings.isEnableDayDream(Configs.currentProjectID)) {
                if (DRFeatureManager.isAndroidXWorkManagerEnabled(Configs.currentProjectID, projectFileBean.fileName)) {
                    addImport("androidx.work.*");
                }

                if (DRFeatureManager.isAndroidXMedia3Enabled(Configs.currentProjectID, projectFileBean.fileName)) {
                    addImport("java.net.*");
                    addImport("androidx.media3.common.*");
                    addImport("androidx.media3.exoplayer.*");
                }

                if (DRFeatureManager.isAndroidXBrowserEnabled(Configs.currentProjectID, projectFileBean.fileName)) {
                ...
                if (DRFeatureManager.isAndroidXCredentialManagerEnabled(Configs.currentProjectID, projectFileBean.fileName)) {
        ...
        if (DRFeatureManager.isGlidetransformationsEnabled(Configs.currentProjectID, projectFileBean.fileName))
            addImport("jp.wasabeef.glide.transformations.*");

        if (DRFeatureManager.isOnBackInvokedCallbackEnabled(Configs.currentProjectID, projectFileBean.fileName))
        ...
        if (buildConfig.g) {
            if (projectFileBean.hasActivityOption(ProjectFileBean.OPTION_ACTIVITY_TOOLBAR) && !projectFileBean.fileName.contains("_fragment") && !DayDreamProjectSettings.getActivityType(Configs.currentProjectID, projectFileBean.fileName).contains("_fragment")) {
                addImport(
